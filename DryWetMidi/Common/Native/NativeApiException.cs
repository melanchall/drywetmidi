using System;

namespace Melanchall.DryWetMidi.Common
{
    /// <summary>
    /// The exception that is thrown when an error occurred in the native MIDI API.
    /// </summary>
    public sealed class NativeApiException : MidiException
    {
        #region Constructor

        /// <summary>
        /// Initializes a new instance of the <see cref="NativeApiException"/>.
        /// </summary>
        public NativeApiException()
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="NativeApiException"/> with the
        /// specified error message.
        /// </summary>
        /// <param name="message">The error message that explains the reason for the exception.</param>
        public NativeApiException(string message)
            : base(message)
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="NativeApiException"/> class with the
        /// specified error message and a reference to the inner exception that is the
        /// cause of this exception.
        /// </summary>
        /// <param name="message">The error message that explains the reason for the exception.</param>
        /// <param name="innerException">The exception that is the cause of the current exception,
        /// or a <c>null</c> reference if no inner exception is specified.</param>
        public NativeApiException(string message, Exception innerException)
            : base(message, innerException)
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="NativeApiException"/> class with the
        /// specified error message and an error code.
        /// </summary>
        /// <param name="message">The error message that explains the reason for the exception.</param>
        /// <param name="mainErrorCode">The error code.</param>
        public NativeApiException(string message, int mainErrorCode, int additionalErrorCode)
            : this(message)
        {
            MainErrorCode = mainErrorCode;
            AdditionalErrorCode = additionalErrorCode;
        }

        #endregion

        #region Properties

        /// <summary>
        /// Gets the code of an error represented by the current <see cref="NativeApiException"/>.
        /// </summary>
        public int? MainErrorCode { get; }

        public int? AdditionalErrorCode { get; }

        #endregion
    }
}
