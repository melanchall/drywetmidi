...
> [!WARNING]
> <os-specific-api Virtual devices API/>
> 
> <advanced-windows-api/>

With DryWetMIDI you can programmatically create virtual MIDI devices with the specified name using [`VirtualDevice.Create`](xref:Melanchall.DryWetMidi.Multimedia.VirtualDevice.Create*) method. In fact, virtual device is an [input](xref:a_dev_input) and an [output](xref:a_dev_output) endpoints paired together in a way that any MIDI event sent to the output endpoint will be immediately transferred back from the virtual device and can be received by an application from its input endpoint.
...
2. in the application you set _MyDevice_ as an output MIDI port, so the application will send MIDI data to the output endpoint of the virtual device;
...
You can even combine virtual devices and [`EndpointsConnector`](xref:a_dev_connector) to broadcast MIDI data to several applications (synthesizers, for example) at the same time:
...
namespace DwmExamples
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var endpointsConnector = rootDevice.InputEndpoint.Connect(
...
You must not explicitly dispose of endpoints of a virtual device. More than that, calling `Dispose` on `virtualDevice.InputEndpoint` and `virtualDevice.OutputEndpoint` will throw an exception. But if you got references to the endpoints by regular methods (for example, by [`InputEndpoint.GetByName`](xref:Melanchall.DryWetMidi.Multimedia.InputEndpoint.GetByName*)), you can call `Dispose` on those references of course.
