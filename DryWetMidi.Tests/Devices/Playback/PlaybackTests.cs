...
namespace Melanchall.DryWetMidi.Tests.Devices
{
    ...
    public sealed class PlaybackTests
    {
        ...
        [Retry(RetriesNumber)]
        [Test]
        public void SetEventCallback_ModifyEvent()
        {
            var velocityShift = (SevenBitNumber)10;

            var eventsToSend = new[]
            {
                new EventToSend(new NoteOnEvent { Velocity = (SevenBitNumber)100 }, TimeSpan.Zero),
                new EventToSend(new NoteOffEvent { Velocity = (SevenBitNumber)110 }, TimeSpan.FromSeconds(1))
            };
            ...
            using (var outputDevice = OutputDevice.GetByName(SendReceiveUtilities.DeviceToTestOnName))
            {
                ...
                using (var playback = new Playback(eventsForPlayback, tempoMap, outputDevice))
                {
                    playback.EventCallback = (midiEvent, time, rawTime) =>
                    {
                        if (!(midiEvent is NoteEvent noteEvent))
                            return midiEvent;

                        noteEvent.Velocity += velocityShift;
                        return noteEvent;
                    };
            ...
            foreach (var events in eventsToSend.Select(e => e.Event).Zip(receivedEvents.Select(e => e.Event), (x, y) => new { X = x, Y = y }))
            {
                var x = events.X;
                var y = events.Y;

                Assert.AreNotSame(y, x, $"Event [{x}] is same as [{y}].");
                Assert.AreNotEqual(y, x, $"Event [{x}] equals to [{y}].");

                var xNoteEvent = (NoteEvent)x;
                var yNoteEvent = (NoteEvent)y;

                Assert.AreEqual(yNoteEvent.Velocity, (SevenBitNumber)(xNoteEvent.Velocity + velocityShift), $"Velocity shift is invalid.");
            }
        }
